  ...
  return (
    <section className="space-y-4 rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-950">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-50">{dict.adminTelemetry.detail.title}</h2>
        ...
        <div className="rounded-xl border border-gray-200 px-3 py-2 text-sm text-gray-600 dark:border-gray-700 dark:text-gray-300">
          {formatTelemetryStatus(execution.status, dict)} / {formatTelemetryKind(execution.kind, dict)}
        </div>
      </div>

      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <Metric label={dict.adminTelemetry.detail.user} value={execution.user_id ?? dict.adminTelemetry.detail.anonymous} />
        <Metric label={dict.adminTelemetry.detail.model} value={execution.resolved_model ?? execution.requested_model} />
        <Metric label={dict.adminTelemetry.detail.cost} value={formatCurrency(execution.usage.cost, locale)} />
        <Metric
      ...
      <JsonBlock title={dict.adminTelemetry.detail.responseText} value={execution.response_text ?? execution.error_message ?? "-"} />
...
function Metric({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-2xl border border-gray-200 p-4 dark:border-gray-800">
      ...
      <div className="mt-2 break-all text-sm font-medium text-gray-900 dark:text-gray-100">{value}</div>
    </div>
  );
}
