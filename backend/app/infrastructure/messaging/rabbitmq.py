...
from app.application.ports import GenerateJobQueue, PublicationJobQueue
...
class RabbitMqPublicationJobQueue(PublicationJobQueue):
    def __init__(self, url: str, queue_name: str) -> None:
        self._url = url
        self._queue_name = queue_name
        self._connection: AbstractRobustConnection | None = None
    ...
    async def publish(self, publication_id: UUID) -> None:
        ...
        try:
            ...
            await channel.default_exchange.publish(
                aio_pika.Message(
                    body=json.dumps({"publication_id": str(publication_id)}).encode("utf-8"),
                    delivery_mode=aio_pika.DeliveryMode.PERSISTENT,
                    content_type="application/json",
                ),
                routing_key=self._queue_name,
        ...
        async with queue.iterator() as iterator:
            async for message in iterator:
                try:
                    ...
                    logger.exception("RabbitMQ publication message handling failed")
    ...
    async def _handle_message(
        ...
        async with message.process(requeue=True):
            payload = json.loads(message.body.decode("utf-8"))
            await handler(UUID(payload["publication_id"]))
